...
const staffApis = {
        ...
        Customers.findOne(
          ...
          (err3, result3) => {
            if (err3)
              ...
              Response.sendResponseWithoutData(
                res,
                resCode.ALREADY_EXIST,
                resMessage.ALL_READY_EXIST_EMAIL
              );
            else {
              var retVal = "";
              const saltRounds = 10;
  ...
  login: (req, res) => {
    ...
    if (!req.body)
      ...
      if (req.body.type != "CUSTOMER") {
        Staff.findOne({ email: req.body.email, status: "ACTIVE" })
          .lean()
          .exec((error, result) => {
              ...
              bcrypt.compare(
                ...
                (err, res1) => {
                  if (res1) {
                    ...
                    // if (!result.jwtToken) {
                    console.log(
                      "secret key is " + config().secret_key,
        ...
        Customers.findOne({ email: req.body.email, status: "ACTIVE" })
          .lean()
          .exec((error, result) => {
            // console.log(error, result, req.body, "sghfsdfsdffsdfh")
            if (error)
  ...
  getProfile: (req, res) => {
    if (!req.body._id)
      ...
      if (req.body.type == "CUSTOMER") {
        Customers.findOne({ _id: req.body._id, status: "ACTIVE" })
          .lean()
          .exec((error, result) => {
        ...
        Staff.findOne({ _id: req.body._id, status: "ACTIVE" })
          .lean()
          .exec((error, result) => {
  ...
  editProfile: (req, res) => {
    if (!req.body._id)
      ...
      if (req.body.type == "CUSTOMER") {
        Customers.findOneAndUpdate(
          { _id: req.body._id, status: "ACTIVE" },
          {
            $set: {
              firstName: req.body.firstName,
              lastName: req.body.lastName,
  ...
  // delete:(req,res)=>{
  //   Staff.findByIdAnd({email:"admin@test.com"}).lean().exec((err,result)=>{
  //     if(err){
  //       Response.sendResponseWithoutData(
  //         res,
  //         resCode.WENT_WRONG,
