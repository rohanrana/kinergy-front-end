...
const serviceApis = {
    ...
    'add': (req, res) => {
        const {title,description,image,status,addBy,category,service,price,duration} = req.body;
        let priceArr = [];
        if (price) {
        ...
        subService.save((err, result) => {
            ...
            if (!err)   
                Response.sendResponseWithData(res, resCode.EVERYTHING_IS_OK, 'Sub Service Save Successfully.', result);
            else
    ...
    'edit': (req, res) => {
        if (!req.body._id) {
            Response.sendResponseWithoutData(res, resCode.WENT_WRONG, 'Please Enter Sub Service Id.');
        } else {
            const {title,description,image,status,addBy,category,service,price,duration} = req.body;
            let priceArr = [];
            if (price) {
            ...
            Service.findOneAndUpdate({ _id: req.body._id }, editData, { new: true }).lean().exec((err, result) => {
                if (!err) {
                    if (req.body.oldImage != 'undefined' && req.body.oldImage != null && req.body.image != 'undefined' && req.body.image != null) {
                        let filePath = 'public/uploads/service/subService' + '/' + req.body.oldImage;
                        fs.unlink(filePath, function(err) {
                            if (!err) console.log('removed');
                        });
                    }
                    Response.sendResponseWithData(res, resCode.EVERYTHING_IS_OK, 'Sub Service Update Successfully.', result);
                } else
                    Response.sendResponseWithoutData(res, resCode.WENT_WRONG, resMessage.WENT_WRONG);
            ...
            Service.findOneAndDelete({ _id: req.body._id }).lean().exec((err, result) => {
                if (!err) {
                    if (req.body.oldImage != 'undefined' && req.body.oldImage != null) {
                        let filePath = 'public/uploads/subService/' + '/' + req.body.oldImage;
                        fs.unlink(filePath, function(err) {
                            if (!err) console.log('removed');
                        });
                    }
