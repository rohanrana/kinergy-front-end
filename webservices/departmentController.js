...
const departmentApis = {
    add: (req, res, next) => {
      ...
      edit: (req, res, next) => {
          if(!req.body._id){
            Response.sendResponseWithoutData(res, resCode.WENT_WRONG, 'Please Enter Department Id.');
          }else{    
        const {
          name,
          image,
        ...
        Department.findOneAndUpdate({ _id: req.body._id }, editData, { new: true }).lean().exec((err, result) => {
