...
const serviceCategoryApis = {
    ...
    serviceCategory.save((err, result) => {
      ...
      if (!err)
        return Response.sendResponseWithData(
          ...
          "Service Category Save Successfully.",
          result
        );
      else
        ...
        .exec((err, result) => {
          if (!err) {
            if (
              ...
              let filePath =
                "public/uploads/service/category" + "/" + req.body.oldImage;
  ...
  getListById: (req, res) => {
    if (!req.body._id) {
      Response.sendResponseWithoutData(
        res,
        resCode.WENT_WRONG,
        "Please Enter Service Category Id."
      );
    } else {
  ...
  delete: (req, res) => {
    if (!req.body._id) {
      ...
      ServiceCategory.findOneAndDelete({ _id: req.body._id })
        .lean()
        .exec((err, result) => {
          if (!err) {
  ...
  changeStatus: (req, res, next) => {
    if (!req.body._id) {
      ...
      Response.sendResponseWithoutData(
        res,
        resCode.WENT_WRONG,
        "Please Enter Service Category Status"
      );
    } else {
      ServiceCategory.findOneAndUpdate(
        { _id: req.body._id },
        ...
        .exec((err, result) => {
          if (!err) {
            Response.sendResponseWithData(
              res,
              resCode.EVERYTHING_IS_OK,
              "Service Category Status Changed Successfully.",
              result
            );
          } else
            Response.sendResponseWithoutData(
  ...
  fileUpload: (req, res, next) => {
    var fileLocation = "public/uploads/service/category";
