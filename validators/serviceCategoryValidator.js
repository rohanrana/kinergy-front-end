...
const generateAddValidation = (req, res, next) => [
    ...
    .custom(value => {
        return ServiceCategory.findOne({ slug: slugify(value, { replacement: '-', remove: undefined, lower: true, strict: true, trim: true }), }).then(serviceCategory => {
            if (serviceCategory) {
    ...
    if (req.file) {
        if (req.file.error != 'undefined' && req.file.error == true) {
            ...
            for (let i = 0; i < Object.keys(req.files).length; i++) {
                ...
                let filePath = 'public/uploads/service/category/' + '/' + req.body[fieldname];
